...
async def crawl_cs_notice_board(url: str) -> List[Dict[str, str]]:
    """
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for item in soup.select("div.basic_tbl_head.tbl_wrap table tbody div.bo_tit a"):
            ...
            result = await crawl_cs_article(session, full_link)
...
async def crawl_cs_article(session, url: str) -> Dict[str, str]:
    """
...
async def get_cs(page_num: int = 1):
    notices = []
    for i in range(page_num):
        notices += await crawl_cs_notice_board("https://cs.yonsei.ac.kr/bbs/board.php" + f'?bo_table=sub5_1&page={i+1}')
        await asyncio.sleep(1)

    return notices


async def upsert_cs(page_num):
    posts = await get_cs(page_num=page_num)
    await upsert_posts(posts, college="컴퓨터과학과")


if __name__ == "__main__":
    notices = asyncio.run(upsert_cs(3))
