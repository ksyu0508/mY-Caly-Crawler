...
async def crawl_me_notice_board(url: str) -> List[Dict[str, str]]:
    """
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for item in soup.select("table.board-table tbody a"):
            ...
            result = await crawl_me_article(session, full_link)
...
async def crawl_me_article(session, url: str) -> Dict[str, str]:
    """
    ...
    base_img_url = "https://me.yonsei.ac.kr"
...
async def get_me(page_num: int = 1):
    notices = []
    for i in range(page_num):
        notices += await crawl_me_notice_board("https://me.yonsei.ac.kr/me/community/notice.do" + f'?mode=list&&articleLimit=10&article.offset={i * 10}')
        await asyncio.sleep(1)

    return notices


async def upsert_me(page_num):
    posts = await get_me(page_num=page_num)
    await upsert_posts(posts, college="기계공학부")


if __name__ == "__main__":
    notices = asyncio.run(upsert_me(3))
