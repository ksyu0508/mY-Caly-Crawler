...
system_prompt = '''
질문한 문서에 대한 분류와 필요한 정보를 추출해야 함. 신청해야되는 기간을 알려주는 경우 신청시작시간과 마감시간을 표시하고, 신청해야되는 기간이 포함되어있지 않을 경우 그냥 기간 표시
place, traget은 문서에 따로 명시되지 않으면 None으로 출력
type의 분류는 다음의 리스트에서 번호 리턴 [1.학교 공지사항, 2.장학금, 3.인턴, 4.채용, 5.행사, 6.대학원, 7.대외활동, 8.동아리, 9.기타]
tags의 분류는 다음의 리스트에서 해당되는 모든 태그의 번호 선택 [1.경영, 2.인사, 3.개발, 4.서비스영업, 5.제품영업, 6.프론트개발, 7.벡엔드개발, 8.교사, 9.임용, 10.냉난방]
해당하는 태그 없으면 빈 리스트 출력
type, tags, period, place, target의 형태로 json 파일로 요약, 형태 이름 바꾸지 말고 그대로 출력
period는 datetime 형태로 출력하며, 시작시간과 끝시간 한쌍만 표시, 기간이 여러개 명시되어있을 경우 신청기간을 표시, 없을 경우 제일 중요한 기간 표시
날짜만 나와있는 경우 시작 시간은 00:00:00, 종료 시간은 23:59:59, period: (start)~(end)의 형태로 출력
'''
