...
async def crawl_civil_notice_board(url: str) -> List[Dict[str, str]]:
    """
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for item in soup.select("table.board-table tbody a"):
            ...
            result = await crawl_civil_article(session, full_link)
...
async def crawl_civil_article(session, url: str) -> Dict[str, str]:
    """
    ...
    base_img_url = "https://civil.yonsei.ac.kr"
...
async def get_civil(page_num: int = 1):
    notices = []
    for i in range(page_num):
        notices += await crawl_civil_notice_board("https://civil.yonsei.ac.kr/civil/community/board_2.do" + f'?mode=list&&articleLimit=10&article.offset={i * 10}')
        await asyncio.sleep(1)

    return notices


async def upsert_civil(page_num):
    posts = await get_civil(page_num=page_num)
    await upsert_posts(posts, college="사회환경시스템공학부")


if __name__ == "__main__":
    notices = asyncio.run(upsert_civil(3))
