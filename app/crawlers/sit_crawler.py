...
async def crawl_sit_notice_board(url: str) -> List[Dict[str, str]]:
    """
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for item in soup.select("table.board-table tbody a"):
            ...
            result = await crawl_sit_article(session, full_link)
...
async def crawl_sit_article(session, url: str) -> Dict[str, str]:
    """
    ...
    base_img_url = "https://sit.yonsei.ac.kr"
...
async def get_sit(page_num: int = 1):
    notices = []
    for i in range(page_num):
        notices += await crawl_sit_notice_board("https://sit.yonsei.ac.kr/sit/community/notice.do" + f'?mode=list&&articleLimit=10&article.offset={i * 10}')
        await asyncio.sleep(1)

    return notices


async def upsert_sit(page_num):
    posts = await get_sit(page_num=page_num)
    await upsert_posts(posts, college="첨단융합공학부")


if __name__ == "__main__":
    notices = asyncio.run(upsert_sit(3))
